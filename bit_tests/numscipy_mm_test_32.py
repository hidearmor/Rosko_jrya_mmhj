...
def main(N, M, K, p, sp_raw, trials, warmups, sp_pattern, algo, filename, bits):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 12: # first argument is path for this script
        print("Format like dis: numscipy_mm_test.py N M K p sp trials warmups sp_pattern algo filename bits")
        sys.exit(1)
    _, N, M, K, p, sp, trials, warmups, sp_pattern, algo, filename, bits = sys.argv
    main(int(N), int(M), int(K), int(p), float(sp), int(trials), int(warmups), sp_pattern, algo, filename, bits)
