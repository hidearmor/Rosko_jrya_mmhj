...
do
	...
	do
		...
		do
			...
			do
				# ./rosko_sgemm_test $n $n $n $p $sp $trials $warmups rosko $FILE $measure
...
python3 plots_p.py $num_sparsity_patterns ${sparsity_pattern[@]} $num_sparsity_values ${sparsity_values[@]} $num_ps ${ps[@]} $n_start $n_end $n_step FILE $nameHype
