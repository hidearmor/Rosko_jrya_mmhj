...
def plot_3D_constant_sparsity(sparsity_patterns, sparsity, ps, ns, titles, results_fname, env_details, fname = "plot_p_3D_constant_sp", time_type="rosko-time"):
    ...
    axes = np.array(axes).flatten()  # Flatten to get a 2D grid for easier indexing
    ...
    for i_spp in range(len(sparsity_patterns)):
        ...
        y1 = np.linspace(rosko_ns.min(), rosko_ns.max(), len(rosko_ns.unique()))
        x2, y2 = np.meshgrid(x1, y1)
        z2 = griddata((rosko_ps, rosko_ns), rosko_times, (x2, y2), method='cubic')
