...
declare -i n=10000
declare -i cores=10
# type for python script
type="row_pattern" # options: random_csr, random_arr, diagonal, row_pattern

# for sp in 70 75 80 85 90 95 98 99;
for sp in 60 70 80 90 95 98 99 99.5 99.7 99.9;
do
	./rosko_sgemm_test_row_pattern 	$n $n $n $cores $sp $trials $warmups rosko $FILE
	python3 numscipy_mm.py $n $n $n $cores $sp $trials $warmups $type numpy_dense $FILE
done
